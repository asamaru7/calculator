...
/***/ "./source/_webpack/js/calc/calc/repayment.js":
/*!***************************************************!*\
  !*** ./source/_webpack/js/calc/calc/repayment.js ***!
  \***************************************************/
/*! no exports provided */
...
/* harmony import */ var _utils_formats__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../utils/formats */ "./source/_webpack/js/calc/utils/formats.js");
 // https://m.search.naver.com/search.naver?where=m&sm=mtb_etc&query=%EC%A4%91%EB%8F%84%EC%83%81%ED%99%98%EC%88%98%EC%88%98%EB%A3%8C%EA%B3%84%EC%82%B0
...
$(function () {
  ...
  new Vue({
    ...
    computed: {
      ...
      remainingPeriodMonth: function remainingPeriodMonth() {
        return parseInt(this.remainingPeriod) * (this.remainingPeriodType === 'y' ? 12 : 1);
      },
    ...
    methods: {
      ...
      _calculate: function _calculate(money, period, interestType, interest, tax) {
        var result = {
          money: money,
          interest: {},
          // 이자
        ...
        if (interestType === 'simple') {
          result['interest']['none'] = Math.round(money * (interest / 12) * period); //단리
        } else if (interestType === 'compound') {
          result['interest']['none'] = Math.round(money * Math.pow(1 + interest / 12, period * 12 / 12) - money); //월복리
        } else if (interestType === 'compound3') {
          result['interest']['none'] = Math.round(money * Math.pow(1 + interest / 4, period * 4 / 12) - money); //3개월복리
        } else if (interestType === 'compound6') {
          result['interest']['none'] = Math.round(money * Math.pow(1 + interest / 2, period * 2 / 12) - money); //6개월복리
        } else if (interestType === 'compound12') {
          result['interest']['none'] = Math.round(money * Math.pow(1 + interest, period / 12) - money); //12개월복리
        } // 이자(세후)


        result['interest']['normal'] = Math.round(result['interest']['none'] * (1 - tax)); // 만기지급액

        result['amount']['none'] = money + result['interest']['none'];
        result['amount']['normal'] = money + result['interest']['normal'];
      ...
      calc: function calc() {
        ...
        var result = this._calculate(money, period, interestType, interest, tax);
...
/***/ 11:
/*!*********************************************************!*\
  !*** multi ./source/_webpack/js/calc/calc/repayment.js ***!
  \*********************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /home/web/static-site/projects/calculator/source/_webpack/js/calc/calc/repayment.js */"./source/_webpack/js/calc/calc/repayment.js");
